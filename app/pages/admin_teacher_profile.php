<?php
// app/pages/admin_teacher_profile.php
...
<body class="bg-gray-50 dark:bg-slate-900 min-h-screen transition-colors duration-200 font-sans text-slate-800 dark:text-slate-200">
    ...
    <!-- Live Campus Map Modal (Simplified for admin view if needed, or kept) -->
    <!-- Assuming admins might want to see where the teacher is if they are monitoring? -->
    <!-- Actually, let's keep it but remove the auto-open param logic to keep it simple -->
    ...
    document.getElementById('campusMapModal').addEventListener('close', function() {
        if (campusMapInstance) {
            ...
            campusMarkers = {};
            if (campusArrowSystem) campusArrowSystem = null;
        }
    ...
    // Using admin location json might be better but public is fine too
        ...
        try {
            ...
            teachers.forEach(t => {
                ...
                if (campusMarkers[t.id]) {
                    ...
                    // Add to arrow system if it exists
                    if (typeof campusArrowSystem !== 'undefined') {
