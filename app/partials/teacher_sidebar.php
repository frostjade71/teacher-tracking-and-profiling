<aside id="sidebar" class="fixed inset-y-0 right-0 z-50 w-64 bg-slate-900 dark:bg-slate-950 text-white transform translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out md:static md:inset-auto md:transform-none flex flex-col border-l md:border-r border-slate-800 rounded-tl-2xl rounded-bl-2xl md:rounded-none">
    ...
    <nav class="flex-1 px-3 py-6 space-y-1">
        ...
        <a href="<?= url('?page=teacher_dashboard') ?>" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg group transition-colors <?= $page === 'teacher_dashboard' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
        ...
        <a href="<?= url('?page=teacher_timetable') ?>" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg group transition-colors <?= $page === 'teacher_timetable' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
            <svg class="w-5 h-5 mr-3 <?= $page === 'teacher_timetable' ? 'text-blue-200' : 'text-slate-400 group-hover:text-white' ?>" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
            Timetable
        </a>

        <a href="<?= url('?page=teacher_subjects') ?>" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg group transition-colors <?= $page === 'teacher_subjects' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' ?>">
